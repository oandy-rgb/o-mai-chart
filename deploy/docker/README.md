...
## 備份到 Cloudflare R2(pg_dump → R2)

`maimai-backup` 服務會定時 `pg_dump` → gzip → 上傳 R2 → 清掉過期備份
(取代 CNPG 的自動備份)。因需 R2 憑證,用 **profile 選用**:

```sh
# 在 .env 填 R2_ACCESS_KEY_ID / R2_SECRET_ACCESS_KEY / R2_ENDPOINT(bucket 預設沿用)
docker compose --profile backup up -d maimai-backup     # 啟用定時備份
docker compose logs -f maimai-backup                    # 看備份日誌

# 手動立即備份一次(測試):
docker compose run --rm -e BACKUP_ONESHOT=1 maimai-backup
```

保留天數/頻率:`.env` 的 `BACKUP_KEEP_DAYS`(預設 30)、`BACKUP_INTERVAL`(秒,預設 86400)。
還原:從 R2 下載某個 `maimai-*.sql.gz`,`gunzip -c 檔 | psql "$DATABASE_URL"`。
